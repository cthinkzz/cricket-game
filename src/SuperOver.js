...
import { updateRuns, updateTarget, updateWickets, updateOvers, updateBalls } from './redux/action'
...
export default function SuperOver () {
  ...
  const battingCards = useSelector((state) => state.filteredBattingCards.filter(card => card.value !== 'new'))
  const shotTimings = useSelector((state) => state.shotTimings.filter(card => card.value !== 'new'))
  ...
  const pickRandomBallingType = () => {
    const randomIndex = Math.floor(Math.random() * (ballingCards.length - 1)) + 1
  ...
  const handleSubmit = (e) => {
    ...
    if (updatedRuns >= targetGiven) {
      setWinner('Australia')
      const newCommentary = `Australia Won the match by ${updateRuns - target} runs`
  ...
  return (
    <Container>
      ...
      <Row className="justify-content-center align-items-center">
        <Col md={4}>
        <h1 className="mt-4" style={{ color: 'blue' }}>INDIA V/S AUSTRALIA Super Over</h1>
          <Image src="../SuperOver.png" style={{ width: '300px', marginLeft: '10px' }} roundedCircle />
        </Col>
        <Col md={4}>
          <h4 className="mt-4" style={{ color: 'red' }}>India is Batting</h4>
          ...
          <Form.Group controlId="target">
            ...
            <Form.Group controlId="batting">
              ...
              <Form.Control as="select" value={batting} onChange={handleBattingCard} required>
